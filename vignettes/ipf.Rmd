...
It is based on the Iterative Proportional Fitting algorithm and gives some flexibility about the details of the implementation.
...
```{r setup}
library(surveysd)
population <- demo.eusilc(2, prettyNames = TRUE)
...
Let's now assume that we want to put constraints on the number of males and females for each year. 
The numbers from the original population can be obtained with `xtabs()`.

```{r}
(gy_distribution <- xtabs(pWeight ~ gender + year, population))
xtabs(pWeight ~ gender + year, pop_sample)
```

Again, we can see that those constraints are not met.
Suppliying the contingency table `gy_distribution` to `ipf()` will again resolve this.

```{r}
pop_sample_c2 <- ipf(pop_sample, conP = list(gy_distribution), w = "pWeight")
xtabs(pWeight ~ gender + year, population)
xtabs(calibWeight ~ gender + year, pop_sample_c2)
```

## Two constraints

Now we assume that we know the distribution of household sizes from registry data.

```{r}
registry_table <- xtabs(pWeight ~ hsize, population)
```

However, those registry data does not provide any information about year or `gender`.
Therefore, the two contingency tables (`gy_distribution` and `registry_table`) have to be specified independently.
This can be done by supplying a list to `conP`

```{r}
pop_sample_c2 <- ipf(pop_sample, conP = list(gy_distribution, registry_table), w = "pWeight")
xtabs(pWeight ~ gender + year, population)
xtabs(calibWeight ~ gender + year, pop_sample_c2)
xtabs(pWeight ~ hsize, population)
xtabs(calibWeight ~ hsize, pop_sample_c2)
```
