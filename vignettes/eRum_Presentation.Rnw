...
\author[Johannes, Gussenbauer]{Johannes Gussenbauer, Alexander Kowarik, Matthias Till}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item Similar approach as proposed by VIJAY
    \item Average bootstrap replicates over waves
  \end{itemize}
    \vspace{.5cm}
...
\begin{itemize}
  \item Use output of \texttt{recalib()} or rectangular data with bootstrap weights.
  \item Function fun is applied on Variable var for, using each bootstrap weight.
  \item Predefined functions available, but custom functions or functions from other packages can be supplied
  \begin{itemize}
    \item Must return double or integer and second argument is weight
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \vspace{.5cm}
  \item Supports a harmonious approach for estimating standard errors on surveysd with rotating panel design
    \begin{itemize}
