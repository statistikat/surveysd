...
\begin{itemize}
  \item EU-SILC and at risk of social exclusion
  \vspace{.5cm}
  \item High quality indicators on national- but estimates on sub-national level have poor accuracy.
    ...
    \item Use administrative data (see povmap) \rightarrow not always available
  \end{itemize}
  \vspace{.5cm}
  \item Difficulty estimating variance of differences between waves
  \vspace{.5cm}
...
\begin{itemize}
  ...
  \item Variance estimation via bootstrap techniques
    \begin{itemize}
      \item Rescaled bootstrap for stratified multistage sampling
    \end{itemize}
    \vspace{.5cm}
  \item Improve accuracy by using multiple (consecutive) waves of the survey
  \begin{itemize}
    \item Average bootstrap replicates over waves (see VIJAY)
  \end{itemize}
    \vspace{.5cm}
...
\begin{itemize}
  \item Estimate errors on subgroups with \texttt{cross\_var} (optional)
  \item Estimate standard errors for differences between years with \texttt{year.diff} (optional)
...
UDB_AT <- recalib(UDB_AT_boot,hid="DB030",weights="RB050",
...
calc.stError(UDB_AT,weights="RB050",
...
calc.stError(UDB_AT,fun="help_gini",
