---
title: "Uncertainty analysis with surveysd"
pkgdown:
  as_is: true
output:
  html_document:
...
```{r}
povertyRate <- calc.stError(dat_boot_calib, var = "povertyRisk", fun = weightedRatio, period.mean = NULL)
totalIncome <- calc.stError(dat_boot_calib, var = "eqIncome", fun = weightedSum, period.mean = NULL)
```

Those functions calculate the ratio of persons at risk of povery (in percent) and the total income.
By default, the results are calculated seperately for each reference period
...
replicates result in using `w1`, `w2`, ..., `w5` instead of `pWeight` when applying the estimator.
...
```{r}
povertyRates <- calc.stError(dat2, var = "povertyRisk", fun = weightedRatio, period.mean = NULL,
                    group = "region")
povertyRates$Estimates
```

The last column with `region = NA` denotes the aggregate over all regions.

### Several grouping variables

The following call of `calc.stError()` demonstrates how several grouping variables can be used to 
split up the data. There are three options.
...
rows in the output is therefore `9 + 2 + 1`.
...
The number of rows in the output is `9*2 + 1`.
...
The number of rows in the output is therefore `9*2 + 9 + 2 + 1`.
