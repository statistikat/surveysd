...
eusilc_no_period <- draw.bootstrap(
  eusilc,
  REP = 2,
  hid = "db030",
  weights = "db090",
...
eusilc <- recalib(
  eusilc,
  conP.var = c("rb090", "age"),
  conH.var = c("db040", "hsize")
)
eusilc_no_period <- recalib(
  eusilc_no_period,
  conP.var = c("rb090", "age"),
  conH.var = c("db040", "hsize")
)
...
test_that("test para - weights, b.weights, year and group", {
  expect_error(
    ...
    "column(s) 'a' specified in 'b.weights' not found in dat",
    fixed = TRUE
  )
  expect_error(
    calc.stError(
  ...
  expect_error(
    ...
    "column(s) 'db090s' specified in 'weights' not found in dat",
    fixed = TRUE
  )
  expect_error(
    calc.stError(
    ...
    "column(s) 'years' specified in 'years' not found in dat",
    fixed = TRUE
  )
...
test_that("test para - period.diff, period.mean", {
  ...
  expect_warning(
    ...
    paste0(
      "Removing 2015-2008 from period.diff - period(s) not",
  ...
  expect_warning(
    calc.stError(
      ...
      var = "povmd60",
      group = c("rb090", "db040"),
      period.diff = c("2015-2010", "2016-2011"),
      period.mean = 3
    ),
  ...
  expect_error(
    calc.stError(
      ...
      var = "povmd60",
      group = c("rb090", "db040"),
      period.diff = c("2015-2011", "2016-2012"),
      period.mean = 3
    ),
    NA
  )
...
test_that("test return", {
  ...
  eusilc.comp <- rbindlist(
    list(
      eusilc[,
        .(V1 = weightedRatio(povmd60, db090), N_true = sum(db090)),
        by = year
      ],
      eusilc[,
        .(V1 = weightedRatio(povmd60, db090), N_true = sum(db090)),
  ...
  eusilc.comp <- merge(
    eusilc.comp,
    eusilc.est$Estimates[, .(year, rb090, db040, N, val_povmd60)]
  )
