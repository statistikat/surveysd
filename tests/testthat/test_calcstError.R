...
test_that("test para - data", {
  expect_error(calc.stError(as.matrix(eusilc), weights = "db090",
                            b.weights = paste0("w", 1:2), period = "year",
...
test_that("test para - weights, b.weights, year and group", {
  expect_error(
    calc.stError(
      eusilc, weights = "db090", b.weights = "a", period = "year",
      var = "povmd60", group = c("rb090", "db040")
    ),
    "Not all elements in b.rep are column names in dat"
  )
  expect_error(
    calc.stError(eusilc, weights = "db090", b.weights = 1:2, period = "year",
                 var = "povmd60", group = c("rb090", "db040")),
    "Not all elements in b.rep are column names in dat"
  )
  ...
  expect_error(
    calc.stError(
      eusilc, weights = "db090s", b.weights = paste0("w", 1:2), period = "year",
      var = "povmd60", group = c("rb090", "db040")),
    "db090s is not a column in dat")
  expect_error(
    calc.stError(
      eusilc, weights = "db090", b.weights = paste0("w", 1:2), period = "years",
      var = "povmd60", group = c("rb090", "db040")),
  ...
  expect_error(
    calc.stError(
      eusilc, weights = "db090", b.weights = paste0("w", 1:2), period = "year",
      var = "povmd60", group = c("rb090s", "db040")),
...
test_that("test para - period.diff, period.mean", {
  ...
  expect_error(calc.stError(
    eusilc, weights = "db090", b.weights = paste0("w", 1:2), period = "year",
    var = "povmd60", group = c("rb090", "db040"), period.mean = NULL), NA)
  expect_warning(
    calc.stError(
      eusilc, weights = "db090", b.weights = paste0("w", 1:2), period = "year",
      var = "povmd60", group = c("rb090", "db040"), period.mean = 4),
    "period.mean must be odd - mean over periods will not be calculated")

  diff.warning <- capture_warnings(calc.stError(
    eusilc, weights = "db090", b.weights = paste0("w", 1:2), period = "year",
    var = "povmd60", group = c("rb090", "db040"), period.diff = "2015-2008"
  ))
...
test_that("test para - bias, size.limit, cv.limit, p", {
  ...
  expect_error(
    calc.stError(eusilc, weights = "db090", b.weights = paste0("w", 1:2),
                 period = "year", var = "povmd60", group = c("rb090", "db040"),
                 bias = "FALSE"),
  ...
  expect_error(
    calc.stError(
      eusilc, weights = "db090", b.weights = paste0("w", 1:2), period = "year",
      var = "povmd60", group = c("rb090", "db040"), size.limit = "10"),
    "size.limit must contain one numeric value")
  expect_error(
    calc.stError(
      eusilc, weights = "db090", b.weights = paste0("w", 1:2), period = "year",
      var = "povmd60", group = c("rb090", "db040"), size.limit = 1:2),
    "size.limit must have length 1")
  expect_error(calc.stError(
    eusilc, weights = "db090", b.weights = paste0("w", 1:2), period = "year",
    var = "povmd60", group = c("rb090", "db040"), size.limit = 50), NA)

  expect_error(
    calc.stError(
      eusilc, weights = "db090", b.weights = paste0("w", 1:2), period = "year",
      var = "povmd60", group = c("rb090", "db040"), cv.limit = 1:10),
    "cv.limit must have length 1")
  expect_error(
    calc.stError(
      eusilc, weights = "db090", b.weights = paste0("w", 1:2), period = "year",
      var = "povmd60", group = c("rb090", "db040"), cv.limit = "1"),
    "cv.limit must contain one numeric value")
  expect_error(calc.stError(
    eusilc, weights = "db090", b.weights = paste0("w", 1:2), period = "year",
    var = "povmd60", group = c("rb090", "db040"), cv.limit = 20), NA)


  expect_error(
    calc.stError(
      eusilc, weights = "db090", b.weights = paste0("w", 1:2), period = "year",
      var = "povmd60", group = c("rb090", "db040"), p = ".5"),
    "p must be a numeric vector")
  expect_error(
    calc.stError(
      eusilc, weights = "db090", b.weights = paste0("w", 1:2), period = "year",
      var = "povmd60", group = c("rb090", "db040"), p = c(.1, .7, 1.2)),
    "Values in p must be between 0 and 1")
  expect_error(calc.stError(
    eusilc, weights = "db090", b.weights = paste0("w", 1:2), period = "year",
    var = "povmd60", group = c("rb090", "db040"), p = c(.1, .7, .9)), NA)
})
