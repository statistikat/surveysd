...
eusilc <- draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata="db040")
eusilc <- recalib(eusilc,hid="db030",weights="db090",b.rep=paste0("w",1:2),period="year",
                  conP.var="rb090",conH.var="db040")

# test input parameter
test_that("test para - data",{
  expect_error(calc.stError(as.matrix(eusilc),weights="db090",b.weights=paste0("w",1:2),period="year",var="povmd60",
                            group=c("rb090","db040")),"dat must be a data.frame or data.table")
  expect_error(calc.stError(eusilc,weights="db090",b.weights=paste0("w",1:2),period="year",var="povmd60",
...
test_that("test para - weights, b.weights, year and group",{
  ...
  expect_error(calc.stError(eusilc,weights="db090",b.weights=1:2,period="year",var="povmd60",
                            group=c("rb090","db040")),"Not all elements in b.rep are column names in dat")

  expect_error(calc.stError(eusilc,weights="db090s",b.weights=paste0("w",1:2),period="year",var="povmd60",
                            group=c("rb090","db040")),"db090s is not a column in dat")
  expect_error(calc.stError(eusilc,weights="db090",b.weights=paste0("w",1:2),period="years",var="povmd60",
                            group=c("rb090","db040")),"years is not a column in dat")

  expect_error(calc.stError(eusilc,weights="db090",b.weights=paste0("w",1:2),period="year",var="povmd60",
                            group=c("rb090s","db040")),"Not all elements on group are column names in dat")

  eusilc.est <- calc.stError(eusilc,weights="db090",b.weights=paste0("w",1:2),period="year",var="povmd60",
...
test_that("test para - period.diff, period.mean",{
  ...
  expect_warning(calc.stError(eusilc,weights="db090",b.weights=paste0("w",1:2),period="year",var="povmd60",
                              group=c("rb090","db040"),period.mean=4),"period.mean must be odd - mean over periods will not be calculated")

  diff.warning <- capture_warnings(calc.stError(eusilc,weights="db090",b.weights=paste0("w",1:2),period="year",var="povmd60",
...
test_that("test para - bias, size.limit, cv.limit, p",{
  ...
  eusilc.bias <- calc.stError(eusilc,weights="db090",b.weights=paste0("w",1:2),period="year",var="povmd60",
  ...
  expect_error(calc.stError(eusilc,weights="db090",b.weights=paste0("w",1:2),period="year",var="povmd60",
                            group=c("rb090","db040"),size.limit=1:2),"size.limit must have length 1")
  expect_error(calc.stError(eusilc,weights="db090",b.weights=paste0("w",1:2),period="year",var="povmd60",
