...
test_that("test para - data",{
  expect_error(draw.bootstrap(as.matrix(eusilc),REP=2,hid="db030",weights="db090",period="year",strata="db040"),
               "dat must be a data.frame or data.table")
  expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata="db040"),NA)
})
...
test_that("test para - REP",{
  ...
  expect_error(draw.bootstrap(eusilc,REP=1:2,hid="db030",weights="db090",period="year",strata="db040"),
               "REP must have length 1")
})

test_that("test para - hid, weights and period",{
  expect_error(draw.bootstrap(eusilc,REP=2,hid="db030s",weights="db090",period="year",strata="db040"),
               "db030s is not a column in dat")
  expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090s",period="year",strata="db040"),
               "db090s is not a column in dat")
  expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="years",strata="db040"),
               "years is not a column in dat")

  eusilc[,year.char :=as.character(year)]
  expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year.char",strata="db040"),
               "year.char is not an integer or numeric column")
})

test_that("test para - strata, cluster and totals",{
  expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata="db0400"),
               "Not all elements in strata are column names in dat")
  expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata="db040",cluster="a"),
               "Not all names in cluster are column names in dat")
  expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata=c("db040","rb090"),cluster=c("db040","db030"),totals=c("N.households")),
               "totals must be specified for each stage")
  expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata=c("db040"),cluster=c("db040","db030")),
               ...
               fixed=TRUE)
  expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata=c("db040"),cluster=c("db030"),totals=c("N.something")),
               "Not all names in totals are column names in dat")
  expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata=c("db040"),cluster=c("db030"),totals=c("N.households.error")),
               "Missing values found in column(s): N.households.error",fixed=TRUE)

  expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata=c("db040"),cluster=c("1")),NA)
  expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata=c("db040"),cluster=c("I")),NA)
  expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata=NULL),NA)
  expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata="I"),NA)
  expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata="1"),NA)
})
...
if(Sys.info()["user"]=="ussenbauer"){
  test_that("test para - bootnames, split and pid",{
    ...
    expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata="db040",split=TRUE),
                 "when split is TRUE pid needs to be a string")

    eusilc[,rb030error:=rb030[1],by=list(year,db030)]
    expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata="db040",split=TRUE,pid="rb030error"),
    ...
    eusilc.split <- draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata="db040",split=TRUE,pid="rb030split")
    ...
    expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata="db040",boot.names="1"),
                 "boot.names must start with an alphabetic character")
    expect_error(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata="db040",boot.names="weight"),NA)
  })
}


test_that("test para - single.PSU",{
  expect_warning(draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata="db040",single.PSU="something"),
...
test_that("test return",{
  dat.boot <- draw.bootstrap(eusilc,REP=2,hid="db030",weights="db090",period="year",strata="db040")
