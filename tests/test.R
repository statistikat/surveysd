...
##############################################
# test SILC data
# with new and old bootstrap procedure
...
set.seed(1234)
dat_boot <- draw.bootstrap(dat=copy(dat_es[RB010==2008]),REP=100,hid="DB030",weights="RB050",strata=c("DB050","I"),cluster="DB060",
                           year="RB010",totals=c("N.cluster","N.households"),split=TRUE,pid="RB030",new.method=TRUE)
...
dat_boot_calib <- recalib(dat=copy(dat_boot),hid="DB030",weights="RB050",
                          year="RB010",b.rep=paste0("w",1:100),conP.var=c("ind3"),conH.var = c("hsize","DB040"),maxIter=200)
dat_boot_old[,ind3:=paste(agex,RB090,sep="-")]
dat_boot_calib_old <- recalib(dat=copy(dat_boot_old),hid="DB030",weights="RB050",
                          year="RB010",b.rep=paste0("w",1:100),conP.var=c("ind3"),conH.var = c("hsize","DB040"),maxIter=200)

dat <- copy(dat_boot_calib)
weights="RB050"
b.weights=paste0("w",1:100)
year="RB010"
var="arose"
cross_var=list(c("DB040","DB100"))
fun="weightedRatio"
year.mean=3
bias=FALSE
add.arg=NULL
...
es_results <- calc.stError(dat_boot_calib,weights="RB050",b.weights=paste0("w",1:100),
                           year="RB010",var="arose",cross_var=list(c("DB040","DB100")))
es_results_old <- calc.stError(dat_boot_calib_old,weights="RB050",b.weights=paste0("w",1:100),
                           year="RB010",var="arose",cross_var=list(c("DB040","DB100")))

merge(es_results$Estimates[,.(RB010,DB040,DB100,val_new=stE_arose)],
      es_results_old$Estimates[,.(RB010,DB040,DB100,val_old=stE_arose)],by=c("RB010","DB040","DB100"))
