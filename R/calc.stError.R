...
calc.stError <- function(
  ...
  if(new_method == TRUE){
    ...
    outx <- help.stError(
      dat = dat, period = period, var = var, weights = weights,
      b.weights = b.weights, fun = fun, relative.share = relative.share, group = group,
      fun.adjust.var = fun.adjust.var, adjust.var = adjust.var,
      period.diff = period.diff, period.mean = period.mean, bias = bias,
      no.na = no.na, size.limit = size.limit, p = p, add.arg = add.arg)
...
help.stError <- function(
  dat, period, var, weights, b.weights = paste0("w", 1:1000), fun, relative.share,
  ...
  if (relative.share) {
    ...
    fun <- dt.eval(
      ...
      ",national.arg) {fun_original(",paste0(formalArgs(fun), collapse = ","),")/national.arg*100}")
  }
