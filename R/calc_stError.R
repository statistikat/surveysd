...
#' @param cross_var list of character vectors containig variables in \code{dat}. For each list entry \code{dat} will be split in subgroups according the containing variables as well as \code{year}
#' and \code{var} and \code{fun} are calculated for each of those groups.  
#' @param add.arg list containing strings
...
calc.stError <- function(dat,weights="hgew",b.weights=paste0("w",1:1000),year="jahr",var="povmd60",fun="weightedRatio",cross_var=list(NULL),add.arg=list(NULL)){
	...
	for(i in 1:length(var[2])){
		out_i <- help.stError(dat=dat,year=year,var=var[i],weights=weights,b.weights=b.weights,fun=fun[i],cross_var=cross_var,no.na=no.na,add.arg=add.arg[[i]])
...
# and calculating standard devation (using the bootstrap replicates) per year and für 3-year rolling means
help.stError <- function(dat,year,var,weights,b.weights=paste0("w",1:1000),fun,cross_var,no.na,add.arg=NULL){
